...
/** Theme */
export const STORAGE_THEME_KEY = '__jaguarete_theme_key';
/** Language */
export const STORAGE_LANG_KEY = '__jaguarete_lng_key';
/** Init Message */
export const STORAGE_INIT_MESSAGE_KET = '__jaguarete_im_key';
/** Flow nodes */
export const FLOW_NODES_KEY = '__jaguarete_static_flow_nodes_key';
...
// Utility function to transform jaguarete-fs:// URLs to HTTP service URLs
export const transformFileUrl = (url: string): string => {
  try {
    if (!url.startsWith('jaguarete-fs://')) {
      return url;
    }

    // Parse the jaguarete-fs:// URL structure
    const parsedUrl = new URL(url);

    if (parsedUrl.protocol !== 'jaguarete-fs:') {
      return url;
    }
