...
class _StubVisTag:
    """Stub for vis display when vis module is not available."""

    async def display(self, content=None):
        """Return empty string as placeholder for vis display."""
        return ""


class _StubVisClient:
    """Stub vis client that returns no-op display objects."""

    def get(self, tag: str) -> _StubVisTag:
        return _StubVisTag()


# Vis module was removed; use stubs for rendering
vis_client = _StubVisClient()
...
class VisAgentMessages(_VisTagMixin):
    pass


class VisAgentPlans(_VisTagMixin):
    pass


class VisAppLink(_VisTagMixin):
    pass
