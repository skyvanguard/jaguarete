...
from jaguarete.core.awel.trigger.base import Trigger
from jaguarete.core.interface.message import ModelMessageRoleType

# TODO: Don't dependent on MixinLLMOperator
from jaguarete.model.operators.llm_operator import MixinLLMOperator
...
class AWELAgentOperator(
    ...
    metadata = ViewMetadata(
        ...
        parameters=[
            Parameter.build_from(
                ...
                "awel_agent",
                AWELAgent,
                description="The jaguarete agent.",
            ),
        ],
        inputs=[
            IOField.build_from(
    ...
    async def map(
        ...
        is_retry_chat = False
        # 检测awel flow的启动位置，如果还没启动当前不执行要匹配到启动点
        # 才开始执行，如果已经启动则当前需要执行
    ...
    async def get_agent(
        ...
        if self.awel_agent.agent_prompt:
            # jaguarete_serve prompt service was removed
            logger.warning(
                "Prompt service is not available; agent_prompt will be ignored."
            )
