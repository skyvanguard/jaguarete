...
from jaguarete.util.configure.manager import (
    ConfigurationManager,
    RegisterParameters,
)
from jaguarete.util.module_utils import import_from_string
from jaguarete.util.parameter_utils import ParameterDescription
...
class MDXDocGenerator:
    ...
    def _parse_class_doc(self, param_cls: Type) -> Tuple[str, str, str]:
        ...
        if hasattr(param_cls, metadata_name):
            from jaguarete.core.awel.flow import ResourceMetadata
...
def generate_docs(config_classes: List[Type], output_path: str):
    ...
    if output_dir.exists():
        backup_dir = Path("/tmp") / "jaguarete_tmp_backup"
...
if __name__ == "__main__":
    ...
    from jaguarete.configs.model_config import ROOT_PATH
