...
DEFAULT_JAGUARETES_DIR = Path.home() / ".jaguaretes"
JAGUARETES_HOME = os.getenv("JAGUARETES_HOME", str(DEFAULT_JAGUARETES_DIR))
JAGUARETES_REPO_HOME = os.getenv("JAGUARETES_REPO_HOME", str(DEFAULT_JAGUARETES_DIR / "repos"))

DEFAULT_REPO_MAP = {
    "skyvanguard/jaguaretes": "https://github.com/skyvanguard/jaguaretes.git",
}
...
JAGUARETES_METADATA_FILE = "jaguaretes.toml"
...
def get_repo_path(repo: str) -> str:
    ...
    return str(Path(JAGUARETES_REPO_HOME) / repo_group / repo_name)


ENV_SIG = _get_env_sig()
# The directory where the jaguaretes package is installed
INSTALL_DIR = Path(JAGUARETES_HOME) / "packages" / ENV_SIG


os.makedirs(JAGUARETES_REPO_HOME, exist_ok=True)
