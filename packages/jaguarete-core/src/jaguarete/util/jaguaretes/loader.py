...
from jaguarete._private.pydantic import BaseModel, ConfigDict, Field, model_validator
from jaguarete.component import BaseComponent, SystemApp
from jaguarete.core.awel import DAG
from jaguarete.core.awel.flow.flow_factory import FlowPanel
from jaguarete.util.jaguaretes.base import (
...
class BasePackage(BaseModel):
    ...
    def load_module_class(
        ...
        from jaguarete.core.awel.dag.loader import _load_modules_from_file
...
class FlowPythonPackage(FlowPackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from jaguarete.core.awel.dag.loader import _process_modules
...
class AgentPackage(BasePackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from jaguarete.agent import ConversableAgent
...
class ResourcePackage(BasePackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from jaguarete.agent.resource import Resource
        from jaguarete.agent.resource.tool.pack import _is_function_tool
...
def parse_package_metadata(package: InstalledPackage) -> BasePackage:
    with open(
        Path(package.root) / JAGUARETES_METADATA_FILE, mode="r+", encoding="utf-8"
    ) as f:
...
def _load_installed_package(path: str) -> List[InstalledPackage]:
    ...
    for package in os.listdir(path):
        ...
        jaguaretes_metadata_file = full_path / JAGUARETES_METADATA_FILE
        if (
            full_path.is_dir()
            and install_metadata_file.exists()
            and jaguaretes_metadata_file.exists()
        ):
...
class JaguaretesLoader(BaseComponent):
    """The loader of the jaguaretes packages"""

    name: str = "jaguarete_jaguaretes_loader"

    def __init__(
        self,
        system_app: Optional[SystemApp] = None,
        install_dir: Optional[str] = None,
        load_jaguaretes_interval: int = 10,
    ):
        """Initialize the JaguaretesLoader."""
        self._system_app = None
        self._install_dir = install_dir or INSTALL_DIR
        self._packages: Dict[str, BasePackage] = {}
        self._load_jaguaretes_interval = load_jaguaretes_interval
    ...
    def before_start(self):
        ...
        schedule.every(self._load_jaguaretes_interval).seconds.do(self.load_package)
    ...
    def load_package(self, is_first: bool = False) -> None:
        ...
        try:
            ...
            if is_first:
                logger.info(
                    f"Found {len(packages)} jaguaretes packages from {self._install_dir}"
                )
            for package in packages:
                self._packages[package.name] = package
                self._register_packages(package)
        except Exception as e:
            logger.warning(f"Load jaguaretes package error: {e}", e)
    ...
    def _register_packages(self, package: BasePackage):
        if package.package_type == "agent":
            from jaguarete.agent import ConversableAgent, get_agent_manager
            ...
            from jaguarete.agent.resource import Resource
            from jaguarete.agent.resource.manage import get_resource_manager
