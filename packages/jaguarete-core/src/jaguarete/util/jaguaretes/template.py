...
from .base import JAGUARETES_METADATA_FILE, TYPE_TO_PACKAGE
...
def create_template(
    ...
    """Create a new flow jaguarete"""
    if jaguaretes_type != "flow":
    ...
    working_directory = os.path.join(working_directory, TYPE_TO_PACKAGE[jaguaretes_type])
    ...
    if jaguaretes_type == "flow":
        ...
        raise ValueError(f"Invalid jaguaretes type: {jaguaretes_type}")
...
def _create_flow_template(
    ...
    _write_jaguaretes_toml(working_directory, name, json_dict)
...
def _create_operator_template(
    ...
    """Create a new operator jaguarete"""
    ...
    if definition_type != "python":
        raise click.ClickException(
            f"Unsupported definition type: {definition_type} for jaguaretes type: "
            f"{jaguaretes_type}"
        )
    # Create project structure
    _create_project_structure(working_directory, name, base_metadata.get("description"))
...
def _write_jaguaretes_toml(working_directory: str, name: str, json_data: dict):
    """Write the jaguaretes.toml file"""

    import tomlkit

    with open(Path(working_directory) / name / JAGUARETES_METADATA_FILE, "w") as f:
...
def _write_manifest_file(working_directory: str, name: str, mod_name: str):
    ...
    manifest = f"""include jaguaretes.toml
...
from jaguarete.core.awel.flow import ViewMetadata, OperatorCategory, IOField, Parameter
...
from jaguarete.agent.util import cmp_string_equal
...
async def _test_agent():
    ...
    \"\"\"
    from jaguarete.model.proxy import OpenAILLMClient
    from jaguarete.agent import AgentContext, AgentMemory, UserProxyAgent, LLMConfig
...
from jaguarete.agent.resource import tool
