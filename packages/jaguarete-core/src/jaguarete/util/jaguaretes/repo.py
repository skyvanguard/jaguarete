...
from .base import (
    JAGUARETES_METADATA_FILE,
    JAGUARETES_REPO_HOME,
    DEFAULT_PACKAGES,
    DEFAULT_REPO_MAP,
    INSTALL_DIR,
...
_DEFAULT_REPO = "skyvanguard/jaguaretes"
...
def list_repos() -> List[str]:
    ...
    """
    repos = set()
    for repo in os.listdir(JAGUARETES_REPO_HOME):
        full_path = os.path.join(JAGUARETES_REPO_HOME, repo)
...
def _get_repo_path(repo: str) -> Path:
    ...
    if len(repo_arr) != 2:
        cl.error(
            ...
            f"eg.(skyvanguard/jaguaretes).",
            exit_code=1,
        )
    return Path(JAGUARETES_REPO_HOME) / repo_arr[0] / repo_arr[1]
...
def _list_repos_details() -> List[Tuple[str, str]]:
    ...
    for repo in repos:
        ...
        full_path = os.path.join(JAGUARETES_REPO_HOME, repo_group, repo_name)
...
def add_repo(repo: str, repo_url: str, branch: str | None = None):
    ...
    if len(repo_arr) != 2:
        cl.error(
            f"Invalid repo name '{repo}', repo name must split by '/', "
            "eg.(skyvanguard/jaguaretes).",
            exit_code=1,
        )
    repo_name = repo_arr[1]
    repo_group_dir = os.path.join(JAGUARETES_REPO_HOME, repo_arr[0])
...
def update_repo(repo: str):
    ...
    repo_path = os.path.join(JAGUARETES_REPO_HOME, repo)
    if not os.path.exists(repo_path):
...
def install(
    ...
    """Install the specified jaguarete from the specified repo

    Args:
        name (str): The name of the jaguarete
        repo (str): The name of the repo
    ...
    if not repo_info:
        cl.error(f"The specified jaguarete '{name}' does not exist.", exit_code=1)
    repo, jaguarete_path = repo_info
    copy_and_install(repo, name, jaguarete_path)


def uninstall(name: str):
    """Uninstall the specified jaguarete
    ...
    if not install_path.exists():
        cl.error(f"The jaguarete '{name}' has not been installed yet.", exit_code=1)
    os.chdir(install_path)
    subprocess.run(["pip", "uninstall", name, "-y"], check=True)
    shutil.rmtree(install_path)
    cl.info(f"Uninstalling jaguarete '{name}'...")
...
def reinstall(
    ...
    """Reinstall the specified jaguarete
...
def inner_uninstall(name: str):
    ...
    install_path = INSTALL_DIR / name
    if not install_path.exists():
        logger.warning(f"The jaguarete '{name}' has not been installed yet.")
        return
    os.chdir(install_path)
    # subprocess.run(["pip", "uninstall", name, "-y"], check=True)
    process = subprocess.Popen(
    ...
    shutil.rmtree(install_path)
    logger.info(f"Uninstalling jaguarete '{name}'...")


def inner_copy_and_install(repo: str, name: str, package_path: Path):
    if not package_path.exists():
        raise ValueError(
            f"The specified jaguarete '{name}' does not exist in the {repo} tap."
        )
    install_path = INSTALL_DIR / name
    if install_path.exists():
        logger.info(
            f"The jaguarete '{name}' has already been installed"
    ...
    try:
        ...
        logger.info(f"Installing jaguaretes '{name}' from {repo}...")
        ...
        logger.info(
            f"Installing jaguaretes '{name}' wheel file {_print_path(wheel_file)}..."
        )
        ...
        logger.info(f"Installed jaguaretes at {_print_path(install_path)}.")
        logger.info(f"jaguaretes '{name}' installed successfully.")
        return True
    except Exception as e:
        if install_path.exists():
            shutil.rmtree(install_path)
...
def copy_and_install(repo: str, name: str, package_path: Path):
    """Install the specified jaguarete from a repository
    ...
    try:
        shutil.copytree(package_path, install_path)
        cl.info(f"Installing jaguaretes '{name}' from {repo}...")
        ...
        wheel_file = wheel_files[0]
        cl.info(f"Installing jaguaretes '{name}' wheel file {_print_path(wheel_file)}...")
        ...
        _write_install_metadata(name, repo, install_path)
        cl.success(f"Installed jaguaretes at {_print_path(install_path)}.")
        cl.success(f"jaguaretes '{name}' installed successfully.")
    except Exception as e:
        if install_path.exists():
            shutil.rmtree(install_path)
        raise e
...
def check_with_retry(
    ...
    """Check the specified jaguarete with retry.

    Args:
        name (str): The name of the jaguarete
        spec_repo (str): The name of the repo
        with_update (bool): Whether to update the repo before installing
        is_first (bool): Whether it's the first time to check the jaguarete
    Returns:
        Tuple[str, Path] | None: The repo and the path of the jaguarete
    """
    repos = _list_repos_details()
    if spec_repo:
    ...
    for repo in repos:
        ...
        for package in DEFAULT_PACKAGES:
            jaguarete_path = repo_path / package / name
            jaguarete_metadata_path = jaguarete_path / JAGUARETES_METADATA_FILE
            if (
                jaguarete_path.exists()
                and jaguarete_path.is_dir()
                and jaguarete_metadata_path.exists()
            ):
                return repo[0], jaguarete_path
    if is_first:
...
def list_repo_apps(repo: str | None = None, with_update: bool = True):
    ...
    if with_update:
        for repo in repos:
            update_repo(repo[0])
    table = Table(title=_("jaguaretes In All Repos"))
    ...
    for repo in repos:
        repo_path = Path(repo[1])
        for package in DEFAULT_PACKAGES:
            jaguarete_path = repo_path / package
            for app in os.listdir(jaguarete_path):
                jaguarete_metadata_path = jaguarete_path / app / JAGUARETES_METADATA_FILE
                if (
                    jaguarete_path.exists()
                    and jaguarete_path.is_dir()
                    and jaguarete_metadata_path.exists()
...
def list_installed_apps():
    ...
    table = Table(title=_("Installed jaguaretes"))
