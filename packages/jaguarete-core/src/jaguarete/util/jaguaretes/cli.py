...
def add_tap_options(func):
    @click.option(
        ...
        default=None,
        required=False,
        help="The repository to install the jaguaretes from",
    )
...
def install(repo: str | None, update: bool, names: list[str]):
    """Install your jaguaretes(operators,agents,workflows or apps)"""
...
def uninstall(names: list[str]):
    """Uninstall your jaguaretes(operators,agents,workflows or apps)"""
...
def reinstall(repo: str | None, update: bool, names: list[str]):
    """Reinstall your jaguaretes(operators,agents,workflows or apps)"""
...
@click.option(
    ...
    type=str,
    required=True,
    help="The name you want to give to the jaguarete",
)
@click.option(
    "-l",
    "--label",
    ...
    default=None,
    required=False,
    help="The label of the jaguarete",
)
@click.option(
    "-d",
    "--description",
    ...
    default=None,
    required=False,
    help="The description of the jaguarete",
)
@click.option(
    "-t",
    "--type",
    ...
    default="flow",
    required=False,
    help="The type of the jaguarete",
)
@click.option(
    "--definition_type",
    ...
    default="json",
    required=False,
    help="The definition type of the jaguarete",
)
@click.option(
    "-C",
    "--directory",
    ...
    default=None,
    required=False,
    help="The working directory of the jaguarete(defaults to the current directory).",
)
def new_jaguaretes(
    name: str,
    label: str | None,
    ...
    """New a jaguaretes module structure"""
    if not label:
        # Set label to the name
        default_label = name.replace("-", " ").replace("_", " ").title()
        label = click.prompt(
            "Please input the label of the jaguarete", default=default_label
        )
    if not description:
        # Read with click
        description = click.prompt(
            "Please input the description of the jaguarete", default=""
        )
    if not directory:
        # Set directory to the current directory(abs path)
        directory = click.prompt(
            "Please input the working directory of the jaguarete",
