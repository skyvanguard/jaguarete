...
if TYPE_CHECKING:
    from jaguarete._private.pydantic import BaseModel
...
class EnvArgumentParser:
    ...
    def _kwargs_to_env_key_value(
        kwargs: Dict, prefix: str = "__jaguarete_gunicorn__env_prefix__"
    ) -> Dict[str, str]:
        return {prefix + k: str(v) for k, v in kwargs.items()}

    @staticmethod
    def _read_env_key_value(
        prefix: str = "__jaguarete_gunicorn__env_prefix__",
    ) -> List[str]:
        env_args = []
...
def _get_parameter_descriptions(
    ...
    from jaguarete.util.configure.manager import ConfigurationManager
...
def _get_base_model_descriptions(model_cls: "BaseModel") -> List[ParameterDescription]:
    from jaguarete._private import pydantic
