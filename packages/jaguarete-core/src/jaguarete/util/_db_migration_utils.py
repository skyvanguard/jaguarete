...
```commandline
jaguarete db migration upgrade
```
...
```commandline
jaguarete db migration clean -y
```
...
```commandline
jaguarete db migration clean --drop_all_tables -y --confirm_drop_all_tables
```
or 
```commandline
rm -rf pilot/meta_data/alembic/versions/*
rm -rf pilot/meta_data/alembic/jaguarete.db
```

If your database is a shared database, and you run Jaguarete in multiple instances, you \
...
and use `jaguarete db migration` command to manage migration scripts.
"""
...
def _check_database_migration_status(alembic_cfg: AlembicConfig, engine: Engine):
    ...
    If your database is a shared database, and you run Jaguarete in multiple instances,
...
def _ddl_init_and_upgrade(
    ...
    if disable_alembic_upgrade:
        ...
        warn_msg = (
            ...
            "(`python jaguarete/app/jaguarete_server.py "
            "--disable_alembic_upgrade`).\n"
            "we suggest you to use `jaguarete db migration` to initialize and upgrade "
            "database metadata with alembic, "
            "your can run `jaguarete db migration --help` to get more information."
        )
        logger.warning(warn_msg)
    from jaguarete.storage.metadata.db_manager import db
