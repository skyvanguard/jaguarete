...
from jaguarete.rag.retriever.base import BaseRetriever, RetrieverStrategy
from jaguarete.rag.retriever.rerank import DefaultRanker, Ranker
from jaguarete.rag.retriever.rewrite import QueryRewrite
from jaguarete.storage.base import IndexStoreBase
from jaguarete.storage.vector_store.filters import MetadataFilters
from jaguarete.util.chat_util import run_async_tasks
from jaguarete.util.tracer import root_tracer
...
class EmbeddingRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        Examples:
            .. code-block:: python
                ...
                from jaguarete.storage.vector_store.connector import VectorStoreConnector
                from jaguarete_ext.storage.vector_store.chroma_store import (
                    ChromaVectorConfig,
                )
                from jaguarete.rag.retriever.embedding import EmbeddingRetriever
                from jaguarete.rag.embedding.embedding_factory import (
    ...
    async def _aretrieve_with_score(
        ...
        if self._query_rewrite:
            with root_tracer.start_span(
                "jaguarete.rag.retriever.embeddings.query_rewrite.similarity_search",
                ...
                context = "\n".join([chunk.content for chunk in chunks])
            with root_tracer.start_span(
                "jaguarete.rag.retriever.embeddings.query_rewrite.rewrite",
        ...
        with root_tracer.start_span(
            "jaguarete.rag.retriever.embeddings.similarity_search_with_score",
            metadata={"query": query, "score_threshold": score_threshold},
        ):
            candidates_with_score = [
        ...
        with root_tracer.start_span(
            "jaguarete.rag.retriever.embeddings.rerank",
            metadata={
                "query": query,
    ...
    async def _similarity_search(
        ...
        """Similar search."""
        with root_tracer.start_span(
            "jaguarete.rag.retriever.embeddings.similarity_search",
            parent_span_id,
            metadata={
                "query": query,
            },
    ...
    async def _similarity_search_with_score(
        ...
        with root_tracer.start_span(
            "jaguarete.rag.retriever.embeddings._do_similarity_search_with_score",
            parent_span_id,
            metadata={
                "query": query,
                "score_threshold": score_threshold,
