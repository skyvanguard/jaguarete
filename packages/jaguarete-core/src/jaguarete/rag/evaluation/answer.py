...
from jaguarete.core import HumanPromptTemplate, ModelMessage
from jaguarete.core.awel import IteratorTrigger, JoinOperator, MapOperator
...
from jaguarete.core.interface.llm import LLMClient, ModelRequest

logger = logging.getLogger(__name__)

ANSWER_RELEVANCY_EVALUATE_PROMPT_TEMPLATE = """
你是一个智能答疑专家, 你的任务是根据用户的问题和已经相关的文档给问答的答案进行严格的\
打分.
你将会得到以下输入信息:
- 用户的问题
- 答案的知识来源，用于评估中的参考.
...
class LLMAnswerEvaluator(Evaluator):
    ...
    async def evaluate(
        ...
        **kwargs,
        ...
        from jaguarete.core.awel import DAG, MapOperator
        ...
        with DAG("relevancy_evaluation_dag"):
            ...
            answer_eva_task = AnswerEvaluatorOperator(
                ...
                llm_client=self.llm_client,  # type: ignore # noqa
            )
