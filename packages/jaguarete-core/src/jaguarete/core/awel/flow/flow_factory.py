...
from jaguarete.core.awel.dag.base import DAG, DAGNode
from jaguarete.core.awel.dag.dag_manager import DAGMetadata
...
class FlowNodeData(BaseModel):
    ...
    id: str = Field(
        ...
        examples=[
            ...
            "resource_jaguarete.model.proxy.llms.chatgpt.OpenAILLMClient_0",
        ],
    )
...
class FlowEdgeData(BaseModel):
    ...
    source: str = Field(
        ...
        examples=["resource_jaguarete.model.proxy.llms.chatgpt.OpenAILLMClient_0"],
    )
...
class _VariablesRequestBase(BaseModel):
    key: str = Field(
        ...
        examples=["jaguarete.model.openai.api_key"],
    )
...
class VariablesRequest(_VariablesRequestBase):
    ...
    For creating a new variable in the Jaguarete.
    """
...
class FlowPanel(BaseModel):
    ...
    source: Optional[str] = Field(
        ...
        examples=["JAGUARETE-WEB", "JAGUARETE-GITHUB"],
    )
...
class FlowFactory:
    ...
    def pre_load_requirements(self, flow_panel: FlowPanel):
        ...
        """
        from jaguarete.util.module_utils import import_from_string
