...
from jaguarete.configs import VARIABLES_SCOPE_FLOW_PRIVATE
from jaguarete.core.awel import BaseOperator, DAGVar, MapOperator
...
from jaguarete.core.awel.flow.flow_factory import (
    FlowData,
    FlowFactory,
    FlowPanel,
    FlowVariables,
        ...
        parameters=[
            Parameter.build_from(
                ...
                ui=ui.UIPasswordInput(
                    key="jaguarete.model.openai.api_key",
                ),
            ),
            Parameter.build_from(
                "Model",
                ...
                ui=ui.UIVariablesInput(
                    key="jaguarete.model.openai.model",
                ),
            ),
            Parameter.build_from(
                "DAG Var 1",
                ...
                options=VariablesDynamicOptions(),
                ui=ui.UIVariablesInput(
                    key="jaguarete.core.flow.params", scope=VARIABLES_SCOPE_FLOW_PRIVATE
                ),
            ),
            Parameter.build_from(
                "DAG Var 2",
...
def json_flow():
    ...
    name_to_parameters_dict = {
        "my_test_variables_operator": {
            "openai_api_key": "${jaguarete.model.openai.api_key:my_key@global}",
            "model": "${jaguarete.model.openai.model:default_model@global}",
            "dag_var1": "${jaguarete.core.flow.params:name1@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
            "dag_var2": "${jaguarete.core.flow.params:name2@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
        }
    }
    ...
    params=[
        {
            "vars": {
                "openai_api_key": {
                    "key": "${jaguarete.model.openai.api_key:my_key@global}",
                    "value": "my_openai_api_key",
                    "value_type": "str",
                    "category": "secret",
                },
                "model": {
                    "key": "${jaguarete.model.openai.model:default_model@global}",
                    "value": "GPT-4o",
                    "value_type": "str",
                },
            }
...
def variables_provider(request):
    pytest.importorskip(
        "jaguarete_serve.flow.api.variables_provider",
        reason="jaguarete_serve package not available",
    )
    from jaguarete_serve.flow.api.variables_provider import BuiltinFlowVariablesProvider
...
async def test_build_flow(json_flow, variables_provider):
    ...
    variables = [
        FlowVariables(
            key="jaguarete.core.flow.params",
            name="name1",
            label="Name 1",
            value="value1",
            value_type="str",
