...
from jaguarete._private.pydantic import BaseModel, model_to_dict
from jaguarete.core.interface.media import MediaContent, MediaContentType, MediaObject
from jaguarete.core.interface.message import ModelMessage, ModelMessageRoleType
from jaguarete.util import BaseParameters
from jaguarete.util.annotations import PublicAPI
from jaguarete.util.model_utils import GPUInfo
...
class ModelOutput:
    ...
    def gen_text_with_thinking(self, new_text: Optional[str] = None) -> str:
        ...
        if self.has_thinking:
            thinking = self.thinking_text or ""
            # Render thinking as fenced block (vis module was removed)
            fence = "`" * 6
            msg = f"{fence}vis-thinking\n{thinking}\n{fence}"
            msg += "\n"
        if new_text:
            msg += new_text
        elif self.has_text:
    ...
    def to_common_messages(
        ...
        Examples:
            .. code-block:: python
                ...
                from jaguarete.core.interface.message import (
                    ModelMessage,
...
class MessageConverter(ABC):
    ...
    Examples:
        ...
        >>> from jaguarete.core.interface.message import ModelMessage, ModelMessageRoleType
        >>> from jaguarete.core.interface.llm import MessageConverter, ModelMetadata
...
class DefaultMessageConverter(MessageConverter):
    ...
    def convert_to_no_system_message(
        ...
        Examples:
            ...
            ...     ModelMessageRoleType,
            ... )
            >>> from jaguarete.core.interface.llm import (
    ...
    def move_last_user_message_to_end(
        ...
        Examples:
            ...
            ...     ModelMessageRoleType,
            ... )
            >>> from jaguarete.core.interface.llm import DefaultMessageConverter
            >>> messages = [
            ...     ModelMessage(
            ...         role=ModelMessageRoleType.SYSTEM,
            ...         content="You are a helpful assistant",
...
class LLMClient(ABC):
    ...
    def __call__(
        ...
        from jaguarete.util import get_or_create_event_loop
