...
from jaguarete.util.singleton import Singleton

if TYPE_CHECKING:
    from jaguarete.component import SystemApp
...
class Config(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        ...
        self.JAGUARETE_WEBSERVER_PORT = int(os.getenv("JAGUARETE_WEBSERVER_PORT", 5670))
        ...
        from jaguarete.core._private.prompt_registry import PromptTemplateRegistry
        ...
        # jaguarete meta info database connection configuration
        ...
        self.LOCAL_DB_NAME = os.getenv("LOCAL_DB_NAME", "jaguarete")
        ...
        # Log level
        self.JAGUARETE_LOG_LEVEL = os.getenv("JAGUARETE_LOG_LEVEL", "INFO")
        ...
        # global jaguarete api key
        self.API_KEYS = os.getenv("API_KEYS", None)
        self.ENCRYPT_KEY = os.getenv("ENCRYPT_KEY", "your_secret_key")

        # Non-streaming scene retries
        self.JAGUARETE_APP_SCENE_NON_STREAMING_RETRIES_BASE = int(
            os.getenv("JAGUARETE_APP_SCENE_NON_STREAMING_RETRIES_BASE", 1)
        )
        # Non-streaming scene parallelism
        self.JAGUARETE_APP_SCENE_NON_STREAMING_PARALLELISM_BASE = int(
            os.getenv("JAGUARETE_APP_SCENE_NON_STREAMING_PARALLELISM_BASE", 1)
        )
    ...
    @property
    def local_db_manager(self):
        """Get local DB manager (requires jaguarete_serve package)."""
        raise NotImplementedError(
            "local_db_manager requires the jaguarete_serve package which is not "
            "available. Use a direct database connection instead."
        )
