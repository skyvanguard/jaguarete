...
from jaguarete.model.adapter.base import LLMModelAdapter
from jaguarete.model.adapter.template import ConversationAdapter, PromptType
...
# If some model is not in the blacklist, but it still affects the loading of Jaguarete,
...
class FastChatLLMModelAdapterWrapper(LLMModelAdapter):
    ...
    def get_generate_stream_function(
        ...
        if _IS_BENCHMARK:
            from jaguarete.util.benchmarks.llm.fastchat_benchmarks_inference import (
                generate_stream,
            )
