...
from jaguarete.core.interface.media import MediaProcessor
...
from jaguarete.model.adapter.template import (
    ConversationAdapter,
    ConversationAdapterFactory,
    get_conv_template,
)
from jaguarete.model.base import ModelType, SupportedModel
from jaguarete.model.parameter import WorkerType

logger = logging.getLogger(__name__)


class EmbeddingModelAdapter(ABC):
    """New Adapter for Jaguarete Embedding models"""
...
class LLMModelAdapter(ABC):
    """New Adapter for Jaguarete LLM models"""
    ...
    def _set_conv_converted_messages(
        ...
        if system_messages:
            if len(system_messages) > 1:
                # Compatible with jaguarete complex scenarios, the last system will protect
    ...
    def model_adaptation(
        ...
        # Some model context to jaguarete server
        model_context = {
